...
function CandidateCard({ candidate }: { candidate: Candidate }) {
  return (
    <Link href={`/candidatos/${candidate.slug}`} className="group flex min-h-[350px] flex-col border border-[#102b36]/14 bg-[#fbf8f1] p-6 shadow-[0_16px_50px_rgba(16,43,54,0.07)] transition duration-300 hover:-translate-y-1 hover:border-[#e75037] hover:shadow-[0_22px_60px_rgba(16,43,54,0.12)]">
      <div className="flex items-center justify-between">
        <span className="grid h-14 w-14 place-items-center rounded-full bg-[#102b36] font-display text-xl font-black text-[#f0dfbd]">{candidate.initials}</span>
        <ArrowUpRight className="h-5 w-5 text-[#e75037] transition group-hover:-translate-y-1 group-hover:translate-x-1" />
      </div>
      <p className="mt-8 text-[8px] font-black uppercase tracking-[0.17em] text-[#748285]">En la boleta: {candidate.ballotName}</p>
      <h2 className="mt-3 font-display text-3xl font-black leading-[0.98] tracking-[-0.04em]">{candidate.name}</h2>
      <p className="mt-3 text-[10px] font-bold uppercase tracking-[0.1em] text-[#e75037]">{candidate.role}</p>
      <p className="mt-5 text-sm leading-6 text-[#596b6e]">{candidate.summary}</p>
      <span className="mt-auto border-t border-[#102b36]/12 pt-5 text-[9px] font-black uppercase tracking-[0.13em]">Ver expediente completo</span>
    </Link>
  );
}
  ...
  return (
    <PageShell>
      <Seo
        ...
        schema={{ "@context": "https://schema.org", "@type": "CollectionPage", name: "Candidatos a alcalde de Laredo 2026", url: `${SITE_URL}/candidatos` }}
      />
      <PageHero eyebrow="Elección municipal 2026" title="Conoce a los candidatos." description="Cinco personas buscan dirigir Laredo. Cada expediente usa la misma estructura, separa hechos de promesas y enlaza a las fuentes originales.">
        <Link href="/comparar-candidatos" className="mt-8 inline-flex items-center gap-3 bg-[#e75037] px-6 py-4 text-[10px] font-black uppercase tracking-[0.13em] text-white shadow-[5px_5px_0_#f0dfbd]">Comparar lado a lado <ArrowRight className="h-4 w-4" /></Link>
      </PageHero>
      <section className="paper-texture py-16 sm:py-24">
        <div className="container">
          <Breadcrumbs items={[{ label: "Candidatos" }]} />
...
export function CandidatePage({ slug }: { slug: string }) {
  ...
  return (
    <PageShell>
      ...
      <section className="paper-texture py-16 sm:py-24">
        <div className="container">
          <Breadcrumbs items={[{ label: "Candidatos", href: "/candidatos" }, { label: candidate.name }]} />
          <div className="grid gap-10 lg:grid-cols-[1fr_330px] lg:gap-16">
            ...
            <aside className="space-y-5 lg:sticky lg:top-28 lg:self-start">
              <AdUnit />
              <Link href="/comparar-candidatos" className="group flex items-center justify-between bg-[#e75037] p-5 text-[10px] font-black uppercase tracking-[0.12em] text-white">Comparar candidatos <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" /></Link>
              <ContactMini />
            </aside>
          </div>
        </div>
...
export function CandidateComparison() {
  ...
  return (
    <PageShell>
      <Seo title="Comparación de candidatos a alcalde de Laredo 2026" description="Compara experiencia y posiciones de Victor Treviño, Alyssa Cigarroa, Poncho Casso, JD Gonzalez y Jorge A. Garza en los principales temas de Laredo." path="/comparar-candidatos" keywords={["compare Laredo mayor candidates", "comparar candidatos Laredo 2026", "Laredo mayor election 2026"]} schema={{ "@context": "https://schema.org", "@type": "WebPage", name: "Comparación de candidatos a alcalde de Laredo 2026" }} />
      ...
      <section className="paper-texture py-16 sm:py-24">
        <div className="container">
          <Breadcrumbs items={[{ label: "Candidatos", href: "/candidatos" }, { label: "Comparar" }]} />
          <div className="overflow-x-auto border border-[#102b36]/15 bg-[#fbf8f1] shadow-[0_20px_60px_rgba(16,43,54,0.08)]">
            <table className="w-full min-w-[1080px] border-collapse text-left">
              <thead className="bg-[#102b36] text-white"><tr><th className="p-5 text-[9px] font-black uppercase tracking-[0.15em]">Tema</th>{candidates.map((candidate) => <th key={candidate.slug} className="p-5"><Link href={`/candidatos/${candidate.slug}`} className="font-display text-xl font-black hover:text-[#f0dfbd]">{candidate.name}</Link></th>)}</tr></thead>
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-5">{candidates.map((candidate) => <Link key={candidate.slug} href={`/candidatos/${candidate.slug}`} className="group border border-[#102b36]/15 bg-white p-5"><span className="text-[8px] font-black uppercase tracking-[0.14em] text-[#e75037]">Expediente</span><p className="mt-2 font-display text-xl font-black">{candidate.name}</p><span className="mt-5 flex items-center justify-between text-[9px] font-black uppercase tracking-[0.12em]">Abrir <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" /></span></Link>)}</div>
        </div>
      </section>
    </PageShell>
  );
...
export function IssueHub() {
  return (
    <PageShell>
      <Seo title="Temas de la elección de Laredo 2026" description="Guías no partidistas sobre agua, impuestos, presupuesto, calles, seguridad, transparencia, comercio, vivienda, salud y economía en Laredo." path="/temas" keywords={["Laredo election issues", "temas elecciones Laredo", "Laredo politics 2026"]} schema={{ "@context": "https://schema.org", "@type": "CollectionPage", name: "Temas de la elección de Laredo 2026" }} />
      ...
      <section className="paper-texture py-16 sm:py-24"><div className="container"><Breadcrumbs items={[{ label: "Temas" }]} /><div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{issues.map((issue) => <IssueCard key={issue.slug} issue={issue} />)}</div><div className="mt-10"><AdUnit /></div></div></section>
    </PageShell>
  );
}
...
export function IssuePage({ slug }: { slug: string }) {
  ...
  const articleSchema = {
    ...
    author: { "@type": "Organization", name: "Laredo Mayor" },
    publisher: { "@type": "Organization", name: "Laredo Mayor" },
  ...
  return (
    <PageShell>
      <Seo title={`${issue.spanishTitle}: candidatos y datos de Laredo 2026`} description={issue.summary} path={`/temas/${issue.slug}`} type="article" keywords={issue.keywords} schema={articleSchema} />
      ...
      <section className="paper-texture py-16 sm:py-24">
        <div className="container">
          <Breadcrumbs items={[{ label: "Temas", href: "/temas" }, { label: issue.spanishTitle }]} />
          <div className="grid gap-10 lg:grid-cols-[1fr_330px] lg:gap-16">
            <article className="space-y-16">
              <section>
              ...
              <section>
                ...
                {issue.candidateContext ? (
                  ...
                  <div className="mt-7 grid gap-4 md:grid-cols-2">{candidates.map((candidate) => <Link key={candidate.slug} href={`/candidatos/${candidate.slug}`} className="group border border-[#102b36]/14 bg-[#fbf8f1] p-5"><p className="text-[8px] font-black uppercase tracking-[0.13em] text-[#e75037]">Expediente</p><h3 className="mt-2 font-display text-xl font-black">{candidate.name}</h3><p className="mt-3 text-xs leading-5 text-[#637477]">{candidate.priorities.join(" · ")}</p><span className="mt-4 flex items-center gap-2 text-[8px] font-black uppercase tracking-[0.12em]">Ver fuentes <ArrowRight className="h-3.5 w-3.5 transition group-hover:translate-x-1" /></span></Link>)}</div>
                )}
              </section>
              <SourceList sources={issue.sources} />
            </article>
            <aside className="space-y-5 lg:sticky lg:top-28 lg:self-start"><AdUnit /><Link href="/comparar-candidatos" className="group flex items-center justify-between bg-[#e75037] p-5 text-[9px] font-black uppercase tracking-[0.12em] text-white">Comparar candidatos <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" /></Link><ContactMini /></aside>
          </div>
        </div>
      </section>
    </PageShell>
...
export function VotingHub() {
  return (
    <PageShell>
      ...
      <section className="paper-texture py-16 sm:py-24"><div className="container"><Breadcrumbs items={[{ label: "Cómo votar" }]} /><div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{votingResources.map((resource, index) => <Link key={resource.slug} href={`/votar/${resource.slug}`} className="group flex min-h-[280px] flex-col border border-[#102b36]/14 bg-[#fbf8f1] p-6 shadow-[0_14px_45px_rgba(16,43,54,0.06)] transition hover:-translate-y-1 hover:border-[#e75037]"><div className="flex justify-between"><span className="grid h-10 w-10 place-items-center rounded-full bg-[#102b36] font-mono text-[10px] font-bold text-white">0{index + 1}</span><ArrowUpRight className="h-5 w-5 text-[#e75037] transition group-hover:-translate-y-1 group-hover:translate-x-1" /></div><h2 className="mt-7 font-display text-3xl font-black tracking-[-0.04em]">{resource.spanishTitle}</h2><p className="mt-4 text-sm leading-6 text-[#5b6d70]">{resource.summary}</p><span className="mt-auto border-t border-[#102b36]/10 pt-5 text-[9px] font-black uppercase tracking-[0.12em]">Abrir guía</span></Link>)}</div><div className="mt-10"><AdUnit /></div></div></section>
    </PageShell>
  );
}
...
export function MissingResource() {
  return (
    <PageShell>
      <Seo title="Página no encontrada" description="La página solicitada no existe en Laredo Mayor." path="/404" />
