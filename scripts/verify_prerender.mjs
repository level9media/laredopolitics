...
for (const route of routes) {
  const file = route === "/" ? path.join(root, "dist/public/index.html") : path.join(root, "dist/public", route.replace(/^\//, ""), "index.html");
  try {
...
console.log(`Verified ${routes.length} pre-rendered HTML pages with route-specific language, title, canonical, description, and H1.`);
